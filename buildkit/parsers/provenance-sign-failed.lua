...
-- path (direct sigstore lib, not the cosign CLI wrapper that
-- cosign-sign-failed catches) prints
--   Error: Signing BuildKit provenance blobs failed: <reason>
-- The <reason> is short and inline (e.g. "error creating tlog entry"
-- when sigstore's rekor endpoint is unhappy). Normalize it to a stable
